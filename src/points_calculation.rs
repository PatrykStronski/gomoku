...
fn calculate_diagonal_points_left(board: &[[u8; 15]; 15], x: usize, y: usize, gamer_code: u8) -> u8 {
    let mut points = 0;
    let mut curr_y = y;
    let mut curr_x = x;
...
fn calculate_diagonal_points_right(board: &[[u8; 15]; 15], x: usize, y: usize, gamer_code: u8) -> u8 {
    let mut points = 0;
    let mut curr_y = y;
    let mut curr_x = x;
    while points_valid(curr_x, curr_y) && board[curr_x][curr_y] == gamer_code {
...
pub fn calculate_field_points(board: &[[u8; 15]; 15], x: usize, y: usize, gamer_code: u8) -> u8 {
    let mut points = 0u8;
    if board[x][y] != gamer_code {
        return points;
    }
    let horizontal = calculate_horizontal_points(board, x, y, gamer_code);
    let vertical = calculate_vertical_points(board, x, y, gamer_code);
    let diagonal = calculate_diagonal_points(board, x, y, gamer_code);
